    ...
    static int robotSim(vector<int>& commands, vector<vector<int>>& obstacles) {
        ...
        for (int c : commands) {
            ...
            default:
                ...
                case 0: { // up
                    auto it =
                        upper_bound(XobSet[x].begin(), XobSet[x].end(), y);
                ...
                case 1: { // left
                    auto it = upper_bound(YobSet[y].rbegin(), YobSet[y].rend(),
                                          x, greater<>());
                ...
                case 2: { // down
                    auto it = upper_bound(XobSet[x].rbegin(), XobSet[x].rend(),
                                          y, greater<>());
                ...
                case 3: { // right
                    auto it =
                        upper_bound(YobSet[y].begin(), YobSet[y].end(), x);
